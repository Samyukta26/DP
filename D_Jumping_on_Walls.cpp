...
bool bfs(vector<string> &v, int n, int k)
{
    ...
    while (!q.empty())
    {
        ...
        if (cur[1] + k > n - 1)
            return true;
        if (v[cur[0]][cur[1] + 1] == '-' && cur[1] + 1 >= cur[2] )
        {
            q.push({cur[0], cur[1] + 1, cur[2] + 1});
            v[cur[0]][cur[1] + 1] = 'X';
        }
        if (cur[1] - 1 >= 0 && v[cur[0]][cur[1] - 1] == '-' && cur[1] - 1 >= cur[2] + 1)
        {
            q.push({cur[0], cur[1] - 1, cur[2] + 1});
            v[cur[0]][cur[1] - 1] = 'X';
        }
        if (v[!cur[0]][cur[1] + k] == '-' && cur[1] + k >= cur[2] + 1)
        {
            q.push({!cur[0], cur[1] + k, cur[2] + 1});
            v[!cur[0]][cur[1] + k] = 'X';
        }
    }
    return false;

...
{
    def1(n);
    vector<int>v(n);   vector<int>p(n);
    for(int i=0; i<n; i++) cin>>v[i];
    for(int i=0; i<n; i++) cin>>p[i];
    vector<ll>s;
  ll c=0; ll maxi=INT_MIN;
    for(int i=0; i<n; i++) s.push_back(v[i]);
    ...
    vector<ll>t;
    for(int i=0; i<n; i++) t.push_back(p[i]);
    for(int i=0; i<n; i++) t.push_back(v[i]);
    ll count=0; ll maxii=0;
